        ...
        implements Predicate
{
    ...
    {
        return new ParquetCorruptionException(format("Corrupted statistics for column \"%s\" in Parquet file \"%s\": [%s]", column, id, columnIndex));
    }
            ...
            implements Serializable
    {
        ...
        public boolean canDrop(org.apache.parquet.filter2.predicate.Statistics<T> statistic)
        {
            if (statistic == null) {
                ...
                else if (statistic.getMin() instanceof Binary && isVarcharType(columnDomain.getType())) {
